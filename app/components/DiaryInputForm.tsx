...
export default function DiaryInputForm({ value, onChange, onSubmit, loading, tooShort }: Props) {
  return (
    <div className="fade-up ds-card max-w-[640px] mx-auto p-6 sm:p-8">
      <label htmlFor="diary-text" className="block font-pretendard text-[15px] font-medium text-[#2B2B2B] mb-4">
        오늘 하루 어땠나요?
      </label>
      ...
      <div className="relative rounded-2xl border border-[#E8E3DA] bg-[#FBFAF7] overflow-hidden focus-within:ring-2 focus-within:ring-[#ECE6FF] transition-shadow">
        ...
        <textarea
          ...
          aria-describedby={value.trim().length > 0 && tooShort ? 'diary-text-hint' : undefined}
      ...
      <div className="mt-4 space-y-3">
        {value.trim().length > 0 && tooShort && (
          <p id="diary-text-hint" className="font-pretendard text-[13px] text-[#EB6A6A]">
            10자 이상 적어주세요
          </p>
        )}
        <button
        ...
        >
          {loading ? (
            ...
            <>
              <Stamp size={17} strokeWidth={2} aria-hidden="true" />
              오늘의 감정 기록하기
            </>
          )}
        </button>
